        ...
        <div class="form-group mt-15">
            ...
            <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror" name="category_id"
                ...
                <optgroup label="{{  $category->title }}">
                    ...
                    <option value="{{ $subCategory->id }}" {{ ((!empty($bundle) and $bundle->category_id ==
                        $subCategory->id) or old('category_id') == $subCategory->id) ? 'selected' : '' }}>{{
                        $subCategory->title }}</option>
                    @endforeach
                </optgroup>
                @else
                <option value="{{ $category->id }}" {{ ((!empty($bundle) and $bundle->category_id == $category->id) or
        ...
        <div class="col-12 col-md-3">
            <div class="webinar-category-filters">
                ...
                <div class="form-group mt-10 d-flex align-items-center justify-content-between">
                    <label class="cursor-pointer font-14 text-gray" for="filterOptions{{ $option->id }}">{{
                        $option->title }}</label>
