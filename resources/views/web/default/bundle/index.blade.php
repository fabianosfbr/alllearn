            ...
            <div class="course-content-body user-select-none">
                <div class="course-body-on-cover text-white">
                    ...
                    <span class="d-block font-16 mt-10">{{ trans('public.in') }} <a
                            ...
                            class="font-weight-500 text-decoration-underline text-white">{{ $bundle->category->title
                            }}</a></span>

                    <div class="d-flex align-items-center">
                        @include('web.default.includes.webinar.rate',['rate' => $bundle->getRate()])
                        <span class="ml-10 mt-15 font-14">({{ $bundle->reviews->pluck('creator_id')->count() }} {{
                            trans('public.ratings') }})</span>
                    </div>

                    <div class="mt-15">
                        <span class="font-14">{{ trans('public.created_by') }}</span>
                        <a href="{{ $bundle->teacher->getProfileUrl() }}" target="_blank"
                            class="text-decoration-underline text-white font-14 font-weight-500">{{
                            $bundle->teacher->full_name }}</a>
                    </div>
                </div>
                    ...
                    <ul class="nav nav-tabs bg-secondary rounded-sm p-15 d-flex align-items-center justify-content-between"
                        ...
                        <li class="nav-item">
                            <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'content') ? 'active' : '' }}"
                                id="content-tab" data-toggle="tab" href="#content" role="tab" aria-controls="content"
                                aria-selected="false">{{ trans('product.content') }} ({{
                        ...
                        <li class="nav-item">
                            <a class="position-relative font-14 text-white {{ (request()->get('tab','') == 'reviews') ? 'active' : '' }}"
                                ...
                                aria-selected="false">{{ trans('product.reviews') }} ({{ $bundle->reviews->count() > 0 ?
                                $bundle->reviews->pluck('creator_id')->count() : 0 }})</a>
                        </li>
                    </ul>
        ...
        <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0">
                ...
                <div class="px-20 pb-30">
                    <form action="/cart/store" method="post">
                        ...
                        <div class="form-check mt-20">
                            ...
                            <label class="form-check-label d-flex flex-column cursor-pointer"
                                ...
                                <span class="font-16 font-weight-500 text-dark-blue">{{ $ticket->title }}
                                    @if(!empty($ticket->discount)) ({{ $ticket->discount }}% {{ trans('public.off') }})
                                    @endif</span>
                        ...
                        <div id="priceBox"
                            ...
                            <div class="text-center">
                                @php
                                $priceWithDiscount = handleCoursePagePrice($bundle->price - ($bundle->price *
                                $activeSpecialOffer->percent / 100));
                                @endphp
                        ...
                        <div class="mt-20 d-flex flex-column">
                            @if($hasBought)
                            <button type="button" class="btn btn-primary" disabled>{{ trans('panel.purchased')
                                }}</button>
                            @elseif($bundle->price > 0)
                            <button type="{{ $canSale ? 'submit' : 'button' }}" @if(!$canSale) disabled @endif
                            ...
                            <a href="/subscribes/apply/bundle/{{ $bundle->slug }}"
                                class="btn btn-outline-primary btn-subscribe mt-20 @if(!$canSale) disabled @endif">{{
                                trans('public.subscribe') }}</a>
                            @endif
                            ...
                            <a href="{{ $canSale ? '/bundles/'. $bundle->slug .'/free' : '#' }}"
                                class="btn btn-primary @if(!$canSale) disabled @endif">{{
                                trans('update.enroll_on_bundle') }}</a>
                            @endif
                        </div>

                    </form>

                    @if(!empty(getOthersPersonalizationSettings('show_guarantee_text')) and
            ...
            <div class="rounded-lg shadow-sm mt-35 px-25 py-20">
                ...
                <div class="mt-30">
                    <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                        ...
                        <span class="font-14">{{ convertMinutesToHourAndMinute($bundle->getBundleDuration()) }} {{
                    ...
                    <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                        <div class="d-flex align-items-center">
                            <img src="/assets/default/img/icons/sessions.svg" width="20" alt="">
                            <span class="ml-5 font-14 font-weight-500">Data de início:</span>
                        </div>
                        <span class="font-14">{{ $bundle->start_date->format('j M Y') }}</span>
                    </div>
            ...
            <div class="rounded-lg tags-card shadow-sm mt-35 px-25 py-20">
                ...
                <div class="d-flex flex-wrap mt-10">
                    ...
                    <a href="" class="tag-item bg-gray200 p-5 font-14 text-gray font-weight-500 rounded">{{ $tag->title
                        }}</a>
                    @endforeach
                </div>
            </div>
