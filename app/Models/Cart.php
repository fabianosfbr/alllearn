...
class Cart extends Model
{
    ...
    public function getItemInfo()
    {
        if (empty($this->itemInfo)) {
            ...
            if (!empty($this->webinar_id)) {
            ...
            } elseif (!empty($this->productOrder)) {
                ...
                $info['itemUrl'] = null;
                $info['title'] = 'Reserva de Reunião ' . ((!empty($this->reserveMeeting->student_count) and $this->reserveMeeting->student_count > 1) ? '(' . trans('update.reservation_appointment_student_count', ['count' => $this->reserveMeeting->student_count]) . ')' : '');
